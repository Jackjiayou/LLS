...
from app.utils.data import response_path_list
from app.utils.sample import h26x_client, ne_utils
...
def prepare_req_data(request_data):
    ...
    for media_path, media_name in media_path2name.items():
        ...
        media_status_matches = parse("$..payload.{}.status".format(payload_path_list[1])).find(request_data)
        media_status = [match.value for match in media_status_matches] if media_status_matches else []
        ...
        for media_path, media_content_list in media_list.items():
            ...
            if i <= media_content_loop:
                ...
                if payload_path_list[2] in ["audio", "video"]:
                    tm = parse("$.payload.{}.timestamp".format(payload_path_list[1])).find(request_data)
                    if multi_mode or tm:
