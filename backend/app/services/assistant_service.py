...
class AssistantService:
    ...
    def process_text_message(self, text: str, scene_id: int, user_id: str, conversation_id: str, history_messages: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        ...
        try:
            # 获取用户路径
            paths = self._get_user_paths(user_id, conversation_id)
            ...
            # 添加当前用户消息
            current_messages = history_messages + [{"from": "user", "text": text}]
            
            # 使用AI生成回复
            chat_msg = getds.get_messages_ai(json.dumps(current_messages))
            robot_words = getds.get_response_qwen(chat_msg)
            ...
            traceback.print_exc()
            raise Exception(f"处理文本消息失败: {str(e)}")
